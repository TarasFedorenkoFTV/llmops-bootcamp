...
Бюджет на **студента** (текстовий формат; виміряні числа — у таблиці нижче):
читання ~45–49 хв/тиждень, 2 guided lab ~25 хв сумарно, домашнє ≤3 год —
разом 4–5 год на тиждень при стелі 5–7. Правило: якщо deliverable не робиться
за 2–3 год після lab — спростити або перенести в optional. Час нижче — це
домашня частина **після** lab.

```mermaid
flowchart LR
  W["Тиждень студента<br/>4–5 год (стеля 5–7)"] --> M["Читання<br/>~45–49 хв"]
  W --> L["Guided lab ×2<br/>~25 хв"]
...
> Числа таблиці — **орієнтовні**: це знімок скриптового підрахунку прози на
> певний момент. Абсолютні значення залежать від методу підрахунку (що
> вважати «прозою» — з кодом, картою, ДЗ-блоком чи без) і дрейфують із кожною
> правкою текстів, тому не є контрактною межею. Канонічний перерахунок робиться
> одним скриптом безпосередньо перед передачею (див. HANDOFF).

Головне, що з цих чисел випливає і від методу не залежить: читання по тижнях
**приблизно рівне** — розкид у межах кількох відсотків, найлегший і найважчий
тиждень за читанням не відрізняються відчутно. Досягнуто не обрізанням, а
двома рішеннями: орієнтація в курсі переїхала з уроку 1 на сторінку змісту, а
тонкі уроки дописані там, де в них справді бракувало тем. (Нерівне —
навантаження за **кодом**, і це свідомо: пік у W4, див. нижче.)

Навантаження за кодом навмисно нерівне і компенсує криву входу: найлегші —
**hw1–hw2** (hw1 — ~60 рядків з нуля при готовому логуванні в стартері: реєстр
і два ендпоінти; hw2 — переважно готовий код з уроків), найважчий — **hw4**
(fallback + HITL), коли людина вже освоїлася зі стеком. Той самий профіль
оголошено студенту в L01 (tip «Про перший тиждень») — ці два описи мають
збігатися.
