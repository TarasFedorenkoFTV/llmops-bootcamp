...

{
  ...
  D.band(s, { x: MX, y: 4.95, w: 12.1, h: 0.85, tone: "card", text: "Останній глосарій курсу — далі тільки практика." });
}
...

{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [2.4, 3.4, 6.3], rowH: 0.56, size: 11.5,
    ...
    rows: [
      ...
      { cells: ["Підтвердити", "яка плитка / який лог?", "fallback_events росте; статуси 503 у лозі"], tone: "acc" },
      { cells: ["Дія", "що робити?", "нічого руками: fallback і деградація вже працюють"], tone: "good" },
...

{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [5.0, 7.1], rowH: 0.52, size: 11.5,
    ...
    rows: [
      ...
      { cells: ["routing mini/strong", "дорога модель — тільки складним; вартість запиту керована"], tone: "acc" },
      { cells: ["eval-гейт у CI", "регресія якості фізично не потрапляє в прод"], tone: "good" },
      { cells: ["HITL-черга", "жодна незворотна дія не виконується без людини"], tone: "acc" },
      { cells: ["вартість на консолі", "витрати видно сьогодні, а не в рахунку через місяць"] },
    ] });
  D.band(s, { x: MX, y: 5.75, w: 12.1, h: 0.75, tone: "warn", label: "Лайфхак",
