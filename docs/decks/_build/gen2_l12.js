...

{
  ...
  D.timeline(s, { x: MX + 0.9, y: 2.45, w: 10.3, marks: [
    { time: "тоді", label: "три рядки коду + 4 питання без відповідей", tone: "crit" },
    { time: "W1", label: "unified log · реєстр промптів", tone: "acc" },
    { time: "W2", label: "routing · cost", tone: "acc" },
    { time: "W3", label: "кеш · tools", tone: "acc" },
    { time: "W4", label: "fallback · guardrail · HITL", tone: "acc" },
    { time: "W5", label: "метрики · evals", tone: "acc" },
    { time: "W6", label: "CI-гейт → контур керування", tone: "good" }] });
...

{
  ...
  D.band(s, { x: MX, y: 5.75, w: 12.1, h: 0.65, tone: "card",
    text: "Кожна ланка — конкретна тема: де спіткнулися, туди й повертаєтесь." });
}
...

{
  ...
  ].forEach(([t, b], i) => D.tile(s, { x: MX + i * 4.05, y: 2.0, w: 3.85, h: 1.8, badge: i + 1, title: t, body: b, tone: "acc" }));
  D.flow(s, { x: MX, y: 4.05, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "провайдер оновив снапшот", tone: "warn" }, { label: "PR: нова версія моделі", tone: "acc", sub: "зміна не приїде сама" },
    { label: "eval-гейт", tone: "crit", sub: "червоний → не мержиться" }, { label: "деплой без сюрпризу", tone: "good" }] });
  D.band(s, { x: MX, y: 5.2, w: 12.1, h: 1.25, tone: "good", label: "Принцип передачі системи",
...
{
  D.divider({ big: "РЕФЛЕКСІЯ", sub: "що це довело · перевірте себе · антипатерни фіналу", notes: N() });
}

{
  ...
  D.consoleMock(s, { x: MX, y: 4.50, w: 5.35, h: 2.0, caption: "точка А · Тема 1",
  ...
  D.consoleMock(s, { x: 7.37, y: 4.50, w: 5.35, h: 2.0, caption: "точка Б · Тема 12",
