...
D.titleSlide({
  ...
  lead: "Модель зробить те, що її переконають зробити. Питання — хто останній у ланцюгу рішень. Сьогодні закриваємо «дірку» з Теми 6: незворотна дія проходить через чергу підтверджень і людину.",
  notes: N(),
});
...

{
  ...
  [["Prompt injection", "вхід намагається переписати правила гри: видати інструкції, зняти обмеження", "crit"],
   ...
   ["Автономні дії", "модель ініціює незворотне — і ніхто не дізнається до наслідків", "crit"],
  ].forEach(([t, b, tone], i) => D.tile(s, { x: MX + i * 4.05, y: 1.9, w: 3.85, h: 1.8, badge: i + 1, title: t, body: b, tone }));
  D.flow(s, { x: MX, y: 3.95, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "вхід користувача" }, { label: "injection-детект", tone: "crit" }, { label: "MaskPii", tone: "crit" },
    { label: "модель", tone: "acc" }, { label: "HITL перед незворотним", tone: "crit" }, { label: "дія", tone: "good" }] });
  D.band(s, { x: MX, y: 4.85, w: 12.1, h: 0.95, tone: "acc",
    text: "Це консенсус галузі, а не надмірна обережність: injection роками очолює галузеві списки ризиків для LLM, поруч із витоком чутливих даних і надмірною автономністю агентів." });
  D.band(s, { x: MX, y: 5.9, w: 12.1, h: 0.75, tone: "good",
    text: "Принцип: жоден із трьох не лікується «кращим промптом»: промпт — прохання, guardrail — механізм." });
}
...

{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [2.6, 5.1, 4.4], rowH: 0.72, size: 11.5,
    ...
    rows: [
      { cells: ["Prompt injection", "проти Ваших інструкцій: підмінити правила, змусити недозволене", "тільки Ви: рамки, розділення, гейт на дії"] },
      ...
      { cells: ["Витік даних", "проти конфіденційності: чужі або внутрішні дані", "Ви: мінімізація, маскування, вихідний фільтр"] },
    ] });
...

{
  ...
  D.band(s, { x: MX, y: 5.3, w: 12.1, h: 1.1, tone: "good", label: "Принцип",
...

{
  ...
  [["Виконання — тільки тут", "після людського «так», і ніде більше"],
   ...
   ["404, а не 409", "заявку споживає approve — повтору немає що захопити; «вже виконано» і «не існувало» нерозрізненні, щоб не давати оракул перебору id"],
...

{
  ...
  D.band(s, { x: 6.87, y: 4.65, w: 5.85, h: 1.6, tone: "warn", label: "Порада",
...
// ─── три родини прямого injection: з чого складати власні кейси (доважок блоку 10) ───
{
  ...
  D.flow(s, { x: MX, y: 4.5, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "«забудь інструкції, тепер ти…»", tone: "crit", w: 3.6 }, { label: "детект / поведінка mock", tone: "acc" },
    { label: "відмова", tone: "good" }, { label: "policy log +1", tone: "good" }] });
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.3, tone: "warn", label: "Опційна доріжка ДЗ тижня 4",
...
// ─── РОЗДІЛЮВАЧ · практика ───
{
  D.divider({ big: "ПРАКТИКА", sub: "стенд наживо — і чесна межа того, що він доводить",
    pill: "Лабораторна: п'ять кроків", notes: N() });
}
...

{
  ...
  s.addText("обидві теми тижня здаються одним PR. Найбільший тиждень курсу за кількістю механізмів — але кожного Ви вже торкнулися в лабах.",
...
D.closingSlide({
  ...
  nextTitle: "Наступний крок → Тема 9 · Observability для LLM-систем",
