...

{
  ...
  [["Трейс", "усе, що сталося в межах одного звернення користувача"],
   ["Спан", "один крок усередині: виклик моделі, інструмент, guardrail — зі своїм початком, тривалістю і статусом"],
  ...
  D.band(s, { x: MX, y: 5.25, w: 12.1, h: 1.15, tone: "good", label: "Закладіть відразу",
    text: "Ідентифікатор народжується на вході, передається у все, що ви кличете, і стоїть у кожному рядку лога — включно з невдалими спробами. Дописати його потім — це переписати всі логи, які вже накопичилися." });
}
...
// ─── стрімінг: чому лог варто мислити ширше за HTTP (доважок блоку 09) ───
{
  ...
  [["Один час стає трьома", "час до першого токена, повна генерація, час до останнього байта — мірити треба перший і третій"],
   ["Помилка посеред успіху", "стрім почався з 200 і обірвався на середині: для HTTP усе гаразд, для користувача — зламано"],
  ...
  D.band(s, { x: MX, y: 4.7, w: 12.1, h: 1.75, tone: "acc", label: "Практичний висновок на сьогодні",
    text: "Мисліть latency_ms і status як «характеристику завершення відповіді», а не як код HTTP. Тоді стрімінг стане розширенням лога, а не його переписуванням." });
}
...
// ─── PII в лозі (доважок блоку 09) ───
{
  ...
  [["Повні тіла — не за замовчуванням", "ознак досить: довжина, токени, finish_reason, спрацювання guardrail, які інструменти кликали"],
  ...
  D.band(s, { x: MX, y: 4.7, w: 12.1, h: 1.75, tone: "good", label: "Правило для кожного нового поля",
    text: "Усе, що ви залогували, ви тепер зобов'язані захищати. Лог — не блокнот, а база даних із тим самим режимом доступу, що й інші ваші дані." });
}
