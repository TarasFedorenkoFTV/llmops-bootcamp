...

{
  ...
  [["01","Тихі інциденти й чотири виміри"],["02","Лог-рядок = джерело всього"],["03","p95, не середнє"],
   ["04","Розтин /observability"],["05","/providers і шість плиток"],["06","Тренд, а не поточне значення"],
...

{
  ...
  D.terms(s, { x: MX, y: 1.95, w: 12.1, cols: 3, rowH: 1.62, items: [
    ...
    { term: "cache-hit / fallback", def: "власні лічильники з Тем 5 і 7" },
...

{
  ...
  [["Якість просіла", "після зміни промпта — HTTP 200"],
   ...
   ["Трафік змістився на слабшу", "працює fallback — HTTP 200"],
...

{
  ...
  D.band(s, { x: 8.35, y: 1.9, w: 4.37, h: 3.0, tone: "good", label: "Принцип",
...

{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [3.0, 5.4, 3.7], rowH: 0.54, size: 11.5,
    ...
    rows: [
      ...
      { cells: ["Вартість сьогодні", "скільки палимо і чи в бюджеті?", "сума cost_usd (Тема 4)"] },
      ...
      { cells: ["Cache-hit %", "чи окупається кеш?", "лічильники (Тема 5)"] },
      { cells: ["Fallback", "чи живемо ми на плані Б?", "лічильник (Тема 7)"] },
    ] });
...

{
  ...
  D.tile(s, { x: MX, y: 4.3, w: 5.85, h: 1.6, title: "Джерело вже є",
    body: "поспіль-збої з результатів CallGateway (тиждень 4); у кого є breaker — його готовий стан", tone: "acc" });
  D.band(s, { x: 6.87, y: 4.3, w: 5.85, h: 1.6, tone: "crit", label: "Пастка кешу",
    text: "Breaker оновлюється лише на реальних викликах — «degraded» триває довше за проблему." });
  D.band(s, { x: MX, y: 6.05, w: 12.1, h: 0.62, tone: "card",
    text: "Статус, прибитий до «ok», — брехня рівно в момент, коли правда потрібна." });
}

{
  const s = D.slide({ num: "06", title: "Тренд важливіший за поточне значення", pill: "absorb", notes: N() });
  ...
  D.band(s, { x: MX, y: 6.16, w: 12.1, h: 0.55, tone: "card", text: "Поточне значення каже «зараз погано». Тренд каже «стає гірше» — і дає час." });
}
...

{
  ...
  [["Симптом, не причина", "«частка успішних упала» — будити завжди; «429 зросло» — причина, будити нечесно"],
   ...
   ["Кожен алерт має інструкцію", "спрацював — що робити першим? Це runbook останньої теми"],
  ].forEach(([t, b], i) => D.tile(s, { x: MX + i * 4.05, y: 1.95, w: 3.85, h: 1.8, badge: i + 1, title: t, body: b, tone: "acc" }));
  D.timeline(s, { x: MX + 0.9, y: 4.45, w: 10.3, marks: [
    { time: "10:00", label: "поодинокі 5xx — фон, не алерт", tone: "good" },
    { time: "10:10", label: "частка успішних падає 3 хв поспіль", tone: "crit" },
    { time: "10:13", label: "алерт: симптом, не причина", tone: "crit" },
    { time: "10:14", label: "runbook: що робити першим", tone: "acc" }] });
  D.band(s, { x: MX, y: 5.5, w: 12.1, h: 1.15, tone: "crit", label: "Типова помилка",
    text: "Окремі 5xx і таймаути — нормальний фон. Алерт — на відхилення в часі. Мітки — тільки осяжні: user_id живе в лозі, не в метриках." });
}

{
  ...
  D.table(s, { x: MX, y: 2.1, w: 12.1, colW: [3.4, 4.4, 4.3], rowH: 0.56, size: 11.5,
    ...
    rows: [
      { cells: ["429 провайдера", "вичерпані ліміти", "розводити трафік, домовлятися про квоту"] },
      { cells: ["timeout інструмента", "зовнішня система не відповіла", "перевіряти інтеграцію, не модель"] },
      { cells: ["відмова guardrail", "injection або заборонений вміст", "це успіх захисту, а не збій"] },
    ] });
...
// ─── PII в лозі (доважок блоку 09) ───
{
  ...
  ].forEach(([t, b], i) => D.tile(s, { x: MX + i * 4.05, y: 2.1, w: 3.85, h: 1.9, badge: i + 1, title: t, body: b, tone: i === 2 ? "crit" : "card" }));
  D.flow(s, { x: MX, y: 4.25, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "запит користувача" }, { label: "ознаки в лог ✓", tone: "good", sub: "довжина · токени · finish_reason · guardrail" },
    { label: "тіло — семпл із TTL", tone: "warn", sub: "окреме сховище" }, { label: "секрети · ключі ✕", tone: "crit", sub: "ніколи" }] });
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.1, tone: "good", label: "Правило для кожного нового поля",
...

{
  ...
  D.flow(s, { x: MX, y: 4.2, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "лог у Postgres" }, { label: "SQL-агрегати", tone: "acc" }, { label: "GET /observability", tone: "acc" },
    { label: "плитки консолі", tone: "good" }, { label: "/metrics → Grafana · опційно", tone: "warn", dashed: true }] });
  D.band(s, { x: MX, y: 5.05, w: 12.1, h: 0.95, tone: "warn", label: "Порада",
    text: "Раз на тиждень звіряйте плитку з SQL руками. Розбіжність — це або баг агрегації, або зміна, про яку Ви не знали." });
  D.band(s, { x: MX, y: 6.1, w: 12.1, h: 0.55, tone: "card",
...

{
  ...
  [["Порахувати руками", "psql: count, p95, error rate по сьогоднішньому дню", false],
   ["Оживити /observability і /providers", "SQL-агрегати + лічильники; «degraded» після N поспіль збоїв", false],
...

{
  ...
  D.windowMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.75, caption: "psql = плитки консолі", lines: [
    ...
    { t: "серія __fail_503:", c: "dim" }, { t: " fallback 3 → 6, error 1.4 → 4 %", c: "crit" }, { t: " uptime: зелений", c: "warn" }] });
...

{
  ...
  D.tile(s, { x: 6.87, y: 1.9, w: 5.85, h: 2.4, title: "Опційно", tone: "warn",
    body: "• таксономія помилок: окремі лічильники за причинами (429 провайдера, timeout інструмента, відмова guardrail)\n\n• сформулювати один SLO і порахувати тижневий бюджет помилок" });
  s.addShape("roundRect", { x: MX, y: 4.6, w: 12.1, h: 1.6, rectRadius: 0.14, fill: { color: P.card }, line: { color: P.acc, width: 1.5 } });
  s.addText("ДЗ тижня 5 — після наступної теми", { x: MX + 0.3, y: 4.8, w: 11.5, h: 0.4, fontFace: F.body, fontSize: 15, bold: true, color: P.acc, margin: 0 });
  s.addText("Воно об'єднує спостережуваність і evals: сьогоднішні ендпоінти вже у Вас в руках; після наступної теми додадуться власні eval-кейси в golden dataset — і тиждень здається одним PR.",
...
D.closingSlide({
  ...
  nextTitle: "Наступний крок → Тема 10 · Golden dataset і eval suite",
  nextBody: "Ви бачите, що система робить. Але «відповіді стали гіршими» жодна з шести плиток не покаже. Наступна тема дає якості число: golden dataset, grader, поріг і exit code — той самий, що на тижні 6 стане гейтом у CI.",
  notes: N(),
});
