...

{
  ...
  D.band(s, { x: MX, y: 6.4, w: 12.1, h: 0.62, tone: "card", text: "Від «не забути прогнати» — до кнопки merge, яка заблокована, поки якість червона." });
}
...

{
  ...
  D.band(s, { x: MX, y: 4.0, w: 12.1, h: 1.2, tone: "acc",
    text: "Це змінює культуру: промпт остаточно стає кодом — з усіма наслідками, від рев'ю до заблокованого мержа." });
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.05, tone: "good", label: "Принцип",
    text: "Усе, що тримається на «не забути», колись буде забуто — питання лише, наскільки дорого. Перевірки, від яких залежить прод, запускаються механізмом." });
}
...

{
  ...
  D.band(s, { x: 8.35, y: 1.95, w: 4.37, h: 2.4, tone: "crit", label: "Пастка: job, а не workflow",
    text: "У списку показується назва job (eval), а не назва workflow (eval-gate). Обравши неіснуючий, ви отримаєте protection, який нічого не блокує." });
  ...
  D.band(s, { x: MX, y: 5.75, w: 12.1, h: 0.65, tone: "card",
    text: "Перевірте налаштування спробою мержу червоного PR — інакше ви не знаєте, чи гейт справді гейт." });
}
...

{
  ...
  D.band(s, { x: MX, y: 4.1, w: 12.1, h: 1.2, tone: "acc",
    text: "В історії PR так і лишилися два червоні прогони, потім зелений. Це не сором — це процес: гейт теж код, і він теж проходить налагодження." });
  D.band(s, { x: MX, y: 5.45, w: 12.1, h: 0.95, tone: "warn", label: "Лайфхак",
    text: "Гейт треба прогнати на свідомо зламаному PR — інакше ви перевірили, що він пропускає, але не що він блокує." });
}

{
  ...
  D.band(s, { x: MX, y: 3.65, w: 12.1, h: 1.6, tone: "crit", label: "Типова помилка",
    text: "Змержити повз червоний гейт «бо терміново». Одна така історія — і гейта немає: за місяць це прапорець, який усі обходять. Якщо гейт помилково червоний — лагодьте гейт, не обходьте." });
  D.band(s, { x: MX, y: 5.4, w: 12.1, h: 1.0, tone: "acc",
    text: "Дзеркальне правило для авторів гейта: він має бути швидким, стабільним і зрозумілим у падінні — інакше обхід стане раціональним." });
}
