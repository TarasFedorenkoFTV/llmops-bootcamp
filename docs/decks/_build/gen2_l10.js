...

{
  ...
  D.terms(s, { x: MX, y: 1.95, w: 12.1, cols: 3, rowH: 1.62, items: [
    ...
    { term: "expect_refusal", def: "очікуємо відмову: red-team кейси з Теми 8" },
...

{
  ...
  D.flow(s, { x: MX, y: 4.05, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "«маленька правка»", tone: "warn" }, { label: "деплою немає · HTTP 200", sub: "моніторинг мовчить" },
    { label: "відповіді стали іншими", tone: "crit", sub: "ніхто не сповістив" }, { label: "eval-прогін: червоне", tone: "good", sub: "число, а не настрій" }] });
  D.band(s, { x: MX, y: 5.15, w: 12.1, h: 1.3, tone: "acc", label: "Навіщо цей шар",
    text: "Це відповідь на третє питання з Теми 1 — «хто помітить, що відповіді стали гіршими?». Метрики Теми 9 кажуть «щось не так»; eval'и кажуть «зламалося саме це»." });
}

{
  ...
  D.layers(s, { x: MX, y: 1.95, w: 12.1, h: 0.66, gap: 0.12, items: [
    ...
    { label: "expect_refusal", body: "очікуємо відмову: red-team кейси з Теми 8", tone: "acc" },
  ] });
...

{
  ...
  D.bars(s, { x: MX, y: 3.85, w: 12.1, labelW: 3.4, noteW: 3.8, rowH: 0.36, gap: 0.12, rows: [
    { label: "FAQ-інтенти", value: 7, note: "«голова» — з лога", tone: "acc" },
    { label: "замовлення · статуси", value: 2, note: "«голова» — з лога", tone: "acc" },
    { label: "ескалації · дії · атаки", value: 1, note: "«хвіст» — зі страхів", tone: "crit" }] });
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.25, tone: "crit", label: "Пишіть expect на суть, а не на форму",
...

{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [4.2, 4.0, 3.9], rowH: 0.72, size: 11.5,
    ...
    rows: [
      { cells: ["Реальні звернення з лога", "формулювання, яких Ви не вигадали б", "Ваш справжній трафік, а не уявлення"] },
      { cells: ["Кожен інцидент — один кейс", "та сама поломка не повториться тихо", "найдешевший спосіб перетворити біль на актив"] },
...

{
  ...
  D.tile(s, { x: 4.72, y: 1.95, w: 8.0, h: 1.4, title: "Чому не всі шість",
    body: "недетермінізм робить поріг 100% крихким: один нестабільний кейс — і червоне ігнорують" });
...

{
  ...
  D.band(s, { x: MX, y: 4.6, w: 12.1, h: 1.0, tone: "acc",
    text: "Регресію з Теми 2 тепер ловить команда, а на тижні 6 — кожен PR." });
...

{
  ...
  D.flow(s, { x: MX, y: 4.05, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "відповідь системи" }, { label: "rule-based: expect / forbid", tone: "good", sub: "безкоштовно, детерміновано" },
    { label: "judge: виклик моделі", tone: "warn", sub: "$ і різний вердикт" }, { label: "калібрування на розмічених", tone: "acc", sub: "лише тоді довіряти" }] });
  D.band(s, { x: MX, y: 5.1, w: 12.1, h: 1.0, tone: "good", label: "Порядок дорослішання",
  ...
  D.band(s, { x: MX, y: 6.2, w: 12.1, h: 0.5, tone: "card",
...

{
  ...
  D.band(s, { x: MX, y: 3.4, w: 12.1, h: 1.35, tone: "warn", label: "Порада",
...

{
  ...
  s.addText("обидві теми тижня здаються одним PR: живі плитки консолі з Теми 9 плюс розширений eval-контур.",
...
D.closingSlide({
  ...
  nextTitle: "Наступний крок → Тема 11 · CI/CD quality gates, canary і rollback",
  nextBody: "Прогін, який треба не забути запустити, не працює. У наступній темі eval-suite переїжджає в CI: гейт на кожен PR, червоний прогін як подарунок, порядок викочування промпта й коду — і критерії відкату, записані до інциденту.",
  notes: N(),
});
