...

{
  ...
  D.band(s, { x: MX, y: 6.4, w: 12.1, h: 0.62, tone: "card", text: "Від «ніби норм» — до числа, яке однакове для стажера, тімліда і CI-раннера." });
}
...

{
  ...
  D.band(s, { x: MX, y: 4.7, w: 12.1, h: 1.4, tone: "good", label: "Принцип",
    text: "Якість — це не відчуття, а перелік тверджень, які можна перевірити. Що не потрапило в датасет — того контракт не обіцяє, і воно зламається мовчки." });
  D.band(s, { x: MX, y: 6.2, w: 12.1, h: 0.6, tone: "card",
    text: "Репрезентативність важливіша за кількість: 20 правильних кейсів корисніші за 200 випадкових." });
}
...

{
  ...
  D.band(s, { x: MX, y: 4.7, w: 12.1, h: 1.35, tone: "good", label: "Принцип",
...

{
  ...
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.05, tone: "good",
    text: "Правило: поріг N−1 при малих наборах, далі — відсоток із округленням угору. Safety-кейси не мають права падати ніколи." });
}

{
  ...
  D.band(s, { x: MX, y: 4.6, w: 12.1, h: 1.0, tone: "acc",
    text: "Регресія, яку на уроці 2 ви ловили очима, тепер ловиться командою — і на тижні 6 ловитиметься автоматично на кожен PR." });
  s.addText("Реальний ключ додасть недетермінізм: кейс, що мигає, — сигнал переписати очікування, а не перезапускати «поки не позеленіє».",
...

{
  ...
  D.band(s, { x: MX, y: 3.4, w: 12.1, h: 1.35, tone: "warn", label: "Лайфхак",
    text: "Два тригери, за якими датасет росте сам собою: щойно щось полагодили — кейс; щойно за щось злякалися на демо — кейс. Обидва займають хвилину, поки все свіже." });
  D.band(s, { x: MX, y: 4.9, w: 12.1, h: 1.3, tone: "acc",
    text: "Датасет, який не росте, старіє разом із продуктом: через квартал він перевіряє систему, якої вже немає." });
}
