...

{
  ...
  D.band(s, { x: MX, y: 5.95, w: 12.1, h: 0.8, tone: "good", label: "Правило теми — measure-first",
    text: "Лічильники hit/miss — в одну тему і в один діф із кешем. Інакше це віра." });
}

{
  ...
  D.flow(s, { x: MX, y: 6.2, w: 12.1, h: 0.48, size: 10, items: [
    { label: "«Як скинути пароль?» ✓", tone: "good" }, { label: "«Де моє замовлення?» ✕", tone: "crit" },
    { label: "результат tool-виклику ✕", tone: "crit" }, { label: "ціни · залишки · статуси ✕", tone: "crit" }] });
}
...

{
  ...
  ].forEach(([t, b], i) => D.tile(s, { x: MX + i * 4.05, y: 2.0, w: 3.85, h: 1.85, badge: i + 1, title: t, body: b, tone: "warn" }));
  D.flow(s, { x: MX, y: 4.1, w: 12.1, h: 0.6, size: 11, items: [
    { label: "100 запитів одночасно", tone: "crit", sub: "холодний ключ" }, { label: "лок або «запит-лідер»", tone: "acc", sub: "решта чекають" },
    { label: "1 виклик моделі", tone: "good", sub: "замість 100" }, { label: "99 віддано з кешу", tone: "good" }] });
  D.band(s, { x: MX, y: 5.3, w: 12.1, h: 1.1, tone: "acc",
...

{
  ...
  [["Зекономлені виклики", "хіти × середня вартість miss із лога = економія в доларах"],
   ["Latency до / після", "хіт ~0 мс проти секунд походу в модель"],
  ...
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.1, tone: "warn", label: "Порада",
...
// ─── дві метрики кешу і межі здорової цифри (доважок блоку 05) ───
{
  ...
  D.table(s, { x: MX, y: 1.8, w: 12.1, colW: [3.0, 5.0, 4.1], rowH: 0.62, size: 11.5,
    ...
    rows: [
      ...
      { cells: ["Кеш провайдера", "токени, обслужені з кешу префікса", "частка кешованих токенів у usage (Тема 4)"] },
    ] });
...

{
  ...
  D.flow(s, { x: MX, y: 3.95, w: 12.1, h: 0.6, size: 11, items: [
    { label: "запит №1 → модель", sub: "відповідь X (будь-яка)" }, { label: "кеш зберіг X", tone: "acc", sub: "лише якщо status 200" },
    { label: "запити №2…n → X", tone: "good", sub: "однакові, миттєві" }, { label: "X = заглушка → X назавжди", tone: "crit", sub: "систематична помилка" }] });
  D.band(s, { x: MX, y: 4.95, w: 12.1, h: 1.05, tone: "good", label: "Принцип: не кешуйте те, у чому не впевнені",
    text: "Пишемо в кеш лише повноцінну відповідь: сьогодні це status == 200." });
  D.band(s, { x: MX, y: 6.12, w: 12.1, h: 0.55, tone: "warn",
...

{
  ...
  D.band(s, { x: MX, y: 3.15, w: 12.1, h: 0.85, tone: "good", label: "Правило контуру",
    text: "Те, що народилося у fallback-гілці (Тема 7), у кеш не потрапляє ніколи." });
...

{
  ...
  D.flow(s, { x: MX, y: 4.1, w: 12.1, h: 0.6, size: 11, items: [
    { label: "«забув пароль, що робити?»", w: 3.6, sub: "нове формулювання" }, { label: "вектор", tone: "acc", sub: "ще один виклик" },
    { label: "схожість ≥ поріг?", tone: "warn", sub: "надто низько — чужа відповідь" }, { label: "відповідь для «Як скинути пароль?»", tone: "good", w: 3.6 }] });
  D.band(s, { x: MX, y: 5.3, w: 12.1, h: 1.15, tone: "acc", label: "Порядок рішень",
...

{
  ...
  D.chatMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.4, msgs: [
    { who: "user", text: "Як скинути пароль?" }, { who: "bot", text: "Надішліть запит на email підтримки — інструкція прийде листом.", badge: "miss · 1.2 s · 84 токени", tone: "warn" },
    { who: "user", text: "Як скинути пароль?" }, { who: "bot", text: "Надішліть запит на email підтримки — інструкція прийде листом.", badge: "hit · 0 мс · 0 токенів", tone: "good" }] });
...

{
  ...
  D.tile(s, { x: MX, y: 1.9, w: 7.9, h: 1.15, title: "Метрика, а не віра", body: "hit/miss ростуть на очах: економія порахована, а не заявлена", tone: "good" });
  D.tile(s, { x: MX, y: 3.2, w: 7.9, h: 1.15, title: "Ключ вирішує коректність", body: "промпт у ключі — і rollback не залишає стару поведінку", tone: "acc" });
  D.tile(s, { x: MX, y: 4.5, w: 7.9, h: 1.15, title: "Межа проведена", body: "персональне й tool-відповіді не кешуються ніколи", tone: "crit" });
  D.consoleMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.75, caption: "консоль · після Теми 5",
    values: [["requests","12"],["cost","$0.0045 / $5"],["p95","—"],["error","—"],["cache-hit","50%"],["fallback","—"]] });
  D.band(s, { x: MX, y: 5.85, w: 12.1, h: 0.8, tone: "card",
    text: "Найдешевший запит — той, якого не було. У наступній темі бот почне діяти, і там межа «tool-відповіді не кешуємо» перестане бути теорією." });
}
...

{
  ...
  s.addText("ДЗ тижня 3 — після наступної теми", { x: MX + 0.3, y: 4.8, w: 11.5, h: 0.4, fontFace: F.body, fontSize: 15, bold: true, color: P.acc, margin: 0 });
  s.addText("Воно об'єднає кеш та інструменти: сьогоднішня частина вже у Вас в руках, після наступної теми додадуться tool-виклики з таймаутом та ідемпотентністю — і тиждень здається одним PR.",
...
D.closingSlide({
  ...
  nextTitle: "Наступний крок → Тема 6 · Tool calls: коли модель починає діяти",
  nextBody: "Досі бот тільки говорив — найгірше, що він міг, це відповісти неправильно. У наступній темі він почне діяти: tool-виклики, зовнішні інтеграції, реальні наслідки. Дія потребує схеми, таймаута та ідемпотентності — а незворотна дія ще й людини, яка скаже «так».",
  notes: N(),
});
