...

{
  ...
  D.band(s, { x: MX, y: 3.65, w: 12.1, h: 1.15, tone: "crit", label: "Пастка: кеш вмикають — і не міряють",
    text: "«Ну, кеш же є, значить економимо». Скільки? На яких запитах? Чи не віддаємо застаріле після зміни промпта? Без цифр кеш — талісман, а не механізм." });
  D.band(s, { x: MX, y: 5.0, w: 12.1, h: 1.35, tone: "good", label: "Правило уроку — measure-first",
    text: "Лічильники hit/miss з'являються разом із кешем: в один урок і в один діф. Кеш без метрики — віра, а не інженерія." });
}

{
  ...
  D.band(s, { x: MX, y: 4.0, w: 5.85, h: 1.85, tone: "acc", label: "Два рівні кешу",
    text: "Наш кеш прибирає виклик цілком. Кеш префікса у провайдера (урок 4) здешевлює виклик зсередини. Вони складаються." });
...

{
  ...
  D.band(s, { x: MX, y: 4.1, w: 12.1, h: 1.35, tone: "crit", label: "Якщо промпта в ключі немає",
    text: "Promote нової версії — кеш далі віддає відповіді старої. Дзеркально і підступніше: rollback полагодив регресію, а користувачі ще годину бачать стару поведінку." });
  D.band(s, { x: MX, y: 5.6, w: 12.1, h: 0.85, tone: "good",
    text: "Промпт у ключі → інвалідація автоматична: змінили версію — старі записи стали недосяжними самі." });
}
...

{
  ...
  D.band(s, { x: MX, y: 4.05, w: 12.1, h: 1.1, tone: "acc", label: "Порядок той самий",
    text: "Механізм і метрика — разом; вітрина — коли з'явиться шар вітрини: на тижні 5 лічильники стануть плиткою Cache-hit." });
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.1, tone: "card", label: "Лайфхак",
    text: "Перш ніж вмикати кеш — витягніть із лога топ повторюваних питань. Топ-10 це 60% трафіку? Окупиться першого ж дня." });
}
...
// ─── дві метрики кешу і межі здорової цифри (доважок блоку 05) ───
{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [3.0, 5.0, 4.1], rowH: 0.62, size: 11.5,
  ...
  D.tile(s, { x: MX, y: 3.75, w: 5.85, h: 1.7, title: "0% — кеш мертвий вантаж",
    body: "шар є, ефекту немає: або питання справді унікальні, або ключ надто чутливий до формулювання", tone: "crit" });
  D.tile(s, { x: 6.87, y: 3.75, w: 5.85, h: 1.7, title: "Підозріло високий — теж сигнал",
    body: "рекордна цифра буває куплена ціною застарілих або чужих відповідей: перевірте, що саме потрапляє в кеш", tone: "warn" });
  D.band(s, { x: MX, y: 5.65, w: 12.1, h: 0.9, tone: "good", label: "Принцип",
    text: "«Не кешувати» — теж інженерне рішення, і ухвалюється воно тими самими цифрами, що й «кешувати». Метрика без розуміння меж — та сама віра, просто з цифрою." });
}

{
  ...
  D.tile(s, { x: 6.87, y: 1.95, w: 5.85, h: 1.8, title: "Кеш консервує помилку",
    body: "одна невдала відповідь роздається всім до кінця TTL: без кешу вона випадкова, з кешем — систематична", tone: "crit" });
  D.band(s, { x: MX, y: 4.0, w: 12.1, h: 1.6, tone: "good", label: "Принцип: не кешуйте те, у чому не впевнені",
    text: "Фільтр перед записом: відповідь дійшла нормально, не заглушка деградації, не відмова фільтра безпеки, не порожня. Сьогодні це status == 200." });
...

{
  ...
  D.band(s, { x: MX, y: 3.9, w: 12.1, h: 1.1, tone: "acc", label: "Чому для LLM це вигідніше, ніж для звичайного API",
    text: "ціна miss'а — секунди очікування і реальні гроші, а «застаріла» інструкція не псується за годину. Виграш: hit ratio росте, p95 не стрибає." });
  D.band(s, { x: MX, y: 5.15, w: 12.1, h: 1.25, tone: "crit", label: "Типова помилка: один TTL на все",
    text: "«Як скинути пароль» може жити добу; статус замовлення — не має права жити й хвилини. TTL — властивість класу запиту, а не налаштування кеша." });
}
