...

{
  ...
  D.tile(s, { x: MX, y: 3.35, w: 5.85, h: 1.75, title: "Вартість — не константа",
    body: "залежить від моделі, довжини контексту, поведінки користувачів і навіть від того, як ви склеюєте промпт" });
  D.tile(s, { x: 6.87, y: 3.35, w: 5.85, h: 1.75, title: "Оцінювання якості теж коштує",
    body: "перевірки на кшталт LLM-as-judge — такі самі токени; не рахувати їх означає недооцінити рахунок", tone: "warn" });
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.0, tone: "good", label: "Принцип",
    text: "Рахується все, що ходить у модель: бойові запити, ретраї, класифікатори і власні перевірки якості." });
}
...

{
  ...
  ].forEach(([t, b], i) => D.tile(s, { x: MX + i * 4.05, y: 2.05, w: 3.85, h: 1.9, badge: i + 1, title: t, body: b, tone: "acc" }));
  D.band(s, { x: MX, y: 4.35, w: 12.1, h: 1.55, tone: "crit", label: "Типова помилка",
    text: "Економити на промпті замість економити на моделі й повторах. «Скоротимо промпт на 30%» — економія в межах найдешевшої ставки; перевести просте питання на дешеву модель — різниця в разах." });
}

{
  ...
  D.code(s, { x: MX, y: 1.9, w: 7.4, h: 2.0, size: 11.5, lines: [
    ...
    [{ t: "{", c: P.darktext }],
    [{ t: '  ["mock-mini"]   = (0.00015m, 0.0006m),', c: "C3E88D" }],
    [{ t: '  ["mock-strong"] = (0.0025m,  0.01m),', c: "C3E88D" }],
    [{ t: "};", c: P.darktext }],
  ] });
  ...
  D.band(s, { x: MX, y: 4.15, w: 12.1, h: 1.0, tone: "acc",
    text: "Пропорції, а не абсолютні цифри, визначають рішення — тому routing з уроку 3 і є головним важелем економії." });
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.0, tone: "card", label: "Чому поруч із Route()",
    text: "Прайс лежить біля точки рішення: ціна моделі — вхідний параметр вибору, а не бухгалтерська довідка." });
}
...

{
  ...
  D.band(s, { x: MX, y: 5.0, w: 12.1, h: 1.35, tone: "good", label: "Принцип: ключ звернення ≠ ключ виклику",
    text: "Групуйте за ідентифікатором звернення, а не за рядком лога — інакше будь-яка цифра вартості систематично занижена." });
}

{
  ...
  D.band(s, { x: MX, y: 4.1, w: 12.1, h: 1.75, tone: "crit", label: "Як чесно порівняти дешеву модель із сильною",
...

{
  ...
  D.code(s, { x: MX, y: 3.6, w: 6.9, h: 1.75, size: 11, lines: [
    ...
    [{ t: "SELECT model, count(*), SUM(cost_usd)", c: "82AAFF" }],
    [{ t: "  FROM requests GROUP BY model;", c: P.darktext }],
    [{ t: "-- за днем: тренд і сплески", c: P.dim }],
    [{ t: "SELECT created_at::date, SUM(cost_usd) …", c: "82AAFF" }],
  ] });
  D.band(s, { x: 7.85, y: 3.6, w: 4.87, h: 1.75, tone: "acc",
    text: "Записали в момент запиту — будь-який зріз коштує один GROUP BY. Не записали — не реконструюєте вже ніколи." });
  D.band(s, { x: MX, y: 5.55, w: 12.1, h: 0.85, tone: "crit", label: "Типова помилка",
    text: "Оптимізувати вартість зміною промпта наосліп: спершу зріз по лозі, потім зміна, потім той самий зріз." });
}

{
  ...
  D.code(s, { x: MX, y: 1.9, w: 7.2, h: 2.2, size: 11.5, lines: [
    ...
    [{ t: "return Results.Json(new {", c: "82AAFF" }],
