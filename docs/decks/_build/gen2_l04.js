...

{
  ...
  D.flow(s, { x: MX, y: 2.05, w: 12.1, h: 0.8, size: 11, items: [
    { label: "ретрай (Тема 7)" }, { label: "fallback (Тема 7)", tone: "warn" }, { label: "tool-цикл (Тема 6)" }, { label: "класифікатор (Теми 3 і 8)" }] });
...

{
  ...
  s.addText("плитка була «—» з Теми 1 — тепер жива", { x: 8.45, y: 3.25, w: 4.0, h: 0.6, fontFace: F.body, fontSize: 11.5, color: P.soft, margin: 0 });
...

{
  ...
  // рівноцінні варіанти; шкала показує, що вмикається послідовно і коли саме.
  {
    ...
    s.addText([{ text: "витрачений бюджет дня:  ", options: { color: P.faint } },
...

{
  ...
  D.windowMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.4, caption: "psql · requests", lines: [
    "SELECT model, cost_usd", "  FROM requests", "  ORDER BY created_at DESC", "  LIMIT 2;", "", { t: " mock-strong | 0.0042", c: "crit" }, { t: " mock-mini   | 0.0003", c: "good" }, "", { t: "ескалація ≈ ×14 дорожча", c: "warn" }] });
...

{
  ...
  [["Завести прайс", "словник цін поруч із Route() у service/Program.cs", false],
   ...
   ["Проговорити пороги", "без коду: що робити при 80% — одним реченням поруч із fallback-порядком", false],
   ["Політика в коді · опційно", "алерт при 80% або деградація на дешевшу модель", true],
  ].forEach(([t, b, opt], i) => {
    const y = 2.0 + i * 0.92;
...

{
  ...
  D.tile(s, { x: MX, y: 1.9, w: 7.9, h: 1.15, title: "Гроші видно одразу", body: "cost_usd з'являється разом із відповіддю, а не в рахунку", tone: "good" });
  D.tile(s, { x: MX, y: 3.2, w: 7.9, h: 1.15, title: "Важіль порахований", body: "різниця маршрутів — у доларах, а не у відчуттях", tone: "acc" });
  D.tile(s, { x: MX, y: 4.5, w: 7.9, h: 1.15, title: "Бюджет — сигнал", body: "плитка порівнює витрату з межею, а не показує абстрактну суму" });
  D.consoleMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.75, caption: "консоль · після Теми 4",
    values: [["requests","10"],["cost","$0.0045 / $5"],["p95","—"],["error","—"],["cache-hit","—"],["fallback","—"]] });
...

{
  ...
  s.addText("обидві теми тижня здаються одним PR: маршрутизація з Теми 3 плюс сьогоднішній облік вартості.",
...
D.closingSlide({
  summary: [
    ...
    "вартість відома в момент відповіді: usage → формула → cost_usd, null замість нуля",
    "звернення ≠ виклик: вартість міряється в одиницях бізнесу — аж до ціни вирішеного звернення",
    "атрибуція важливіша за суму: поля в момент запиту → розслідування сплеску за три GROUP BY",
    "маршрутизація — найбільший важіль вартості; бюджет поруч із витратою — поріг дії",
  ],
  nextTitle: "Наступний крок → Тема 5 · Кешування: точний кеш, метрика і semantic cache",
