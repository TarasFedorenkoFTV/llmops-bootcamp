...
// карта
{
  ...
  [["01","Регресія без коміта"],["02","Промпт — не «просто текст»"],["03","Анатомія промпта"],
   ...
   ["07","Promote і rollback"],["08","Mock і регресія промпта"],["09","Правила версіонування"],
   ["10","Журнал активацій · опційно"],["11","Лабораторна"],["12","Типові помилки"],
...
// глосарій
{
  // Склад звірений із «Ключовими поняттями» затвердженої теми: до шести
  // робочих термінів лабораторної додані fail-visible, snapshot моделі й
  // canary — вони є в начитці й лонгріді, але на слайді їх не було.
  const s = D.slide({ title: "Терміни, якими користуватимемось", pill: "absorb",
    kicker: "Девʼять слів теми — щоб вони не відволікали по ходу",
    notes: N() });
  D.terms(s, { x: MX, y: 1.95, w: 12.1, cols: 3, rowH: 1.20, items: [
    ...
    { term: "prompt injection", def: "дані, які модель прочитала як інструкцію (Тема 8)" },
    { term: "fail-visible", def: "збій видно одразу, а не ховається за «розумним» дефолтом" },
    { term: "snapshot моделі", def: "закріплена версія моделі; перехід — окремий реліз" },
    { term: "canary", def: "порівняння двох версій на живому трафіку (Тема 11)" },
  ] });
}
...
{
  const s = D.slide({ num: "01", title: "Регресія промпта без змін у Git", pill: "absorb", notes: N() });
...
// 02 код vs промпт
{
  const s = D.slide({ num: "02", title: "Промпт як версіонований артефакт", pill: "absorb", notes: N() });
...
// 03 анатомія
{
  const s = D.slide({ num: "03", title: "Структура системного промпта", pill: "absorb",
...
// 04 правила
{
  ...
  D.tile(s, { x: 6.87, y: 3.55, w: 5.85, h: 1.3, title: "Ціна прикладів",
    body: "їдуть у кожному запиті — множаться на весь трафік (Тема 4)", tone: "warn" });
  ...
  D.band(s, { x: MX, y: 5.68, w: 12.1, h: 0.85, tone: "warn", label: "Порада",
...
// 04 injection
{
  const s = D.slide({ num: "04", title: "Змінні в промпті: розмежування інструкцій і даних", pill: "absorb", notes: N() });
...
// 05 реєстр
{
  const s = D.slide({ num: "05", title: "Структура реєстру промптів у базі даних", pill: "absorb", notes: N() });
...
// 06 сервіс читає
{
  const s = D.slide({ num: "06", title: "Отримання активного промпта та логування його версії", pill: "absorb", notes: N() });
  ...
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.0, tone: "good",
    text: "Поле prompt_version, яке з Теми 1 стояло порожнім, від сьогодні заповнене — розслідування якості стає SQL-запитом." });
}

// 07 promote / rollback
{
  const s = D.slide({ num: "07", title: "Promote і rollback через одну операцію активації", pill: "absorb", notes: N() });
  ...
  [["Невідома версія → 404", "чесна відмова, не тихе «ок»: одруківка не деактивує все"],
   ["Слід лишає лог, не реєстр", "UPDATE не веде журналу: хто й коли перемкнув — це audit trail, опційна частина ДЗ"],
...
// 07 A/B
{
  const s = D.slide({ num: "07", title: "Порівняння версій промпта на реальному трафіку", pill: "absorb", notes: N() });
  ...
  D.band(s, { x: MX, y: 5.0, w: 12.1, h: 1.15, tone: "card",
    text: "Активні дві версії; «кому яку» вирішує той самий шар, що обирає модель (Тема 3). У курсі це не обов'язкова доріжка — але механізм у Вас уже буде." });
}

// 08 mock
{
  const s = D.slide({ num: "08", title: "Як mock-провайдер відтворює регресію промпта", pill: "absorb", notes: N() });
...
// 09 дисципліна
{
  const s = D.slide({ num: "09", title: "Правила версіонування промптів", pill: "absorb", notes: N() });
  ...
  ].forEach(([t, b], i) => D.tile(s, { x: MX + i * 4.05, y: 1.9, w: 3.85, h: 1.75, badge: i + 1, title: t, body: b, tone: "good" }));
  D.flow(s, { x: MX, y: 3.95, w: 12.1, h: 0.6, size: 11.5, items: [
    { label: "v1", sub: "історія — не видаляємо" }, { label: "v2", sub: "історія — для diff і відкату" },
    { label: "v3 · активна", tone: "good", sub: "її не редагуємо" }, { label: "v4 · нова", tone: "acc", sub: "навіть одруківка — нова версія" }] });
  D.band(s, { x: MX, y: 5.05, w: 12.1, h: 1.35, tone: "crit", label: "Рев'ю зміни промпта — чесний diff двох body",
...
{
  const s = D.slide({ num: "10", title: "Журнал активацій версій (опційно)", pill: "absorb", opt: true, notes: N() });
...
// лаба
{
  const s = D.slide({ title: "Лабораторна робота: створення реєстру та відтворення регресії", pill: "do", notes: N() });
...
{
  D.divider({ big: "РЕФЛЕКСІЯ", sub: "що це довело · перевірте себе · типові помилки", notes: N() });
}
...
// антипатерни
{
  const s = D.slide({ title: "Типові помилки на початковому етапі", pill: "connect", notes: N() });
...
// ДЗ
{
  ...
  D.tile(s, { x: MX, y: 1.9, w: 5.85, h: 2.2, title: "Обов'язково",
    body: "здати ДЗ тижня 1 одним PR у своєму репозиторії — це рівно те, що ми пройшли за дві теми: у лабах більша частина зроблена руками, вдома довести до чистого стану" });
...
D.closingSlide({
  ...
  nextTitle: "Наступний крок → Тема 3 · Мультипровайдерний gateway і маршрутизація моделей",
  nextBody: "Промпт під контролем — тепер під контроль береться модель. У наступній темі з'являється другий елемент ланцюга і точка рішення: яке питання куди відправити, чому «одна модель на все» дорога, і як розподіл трафіку стає видимим у лозі.",
  notes: N(),
});
