...

{
  ...
  [["Прочитати tool_calls", "і пояснити, чому це прохання, а не подія"],
   ...
   ["Провести межу незворотного", "контракт: через людину (Тема 8)"],
...

{
  ...
  D.terms(s, { x: MX, y: 1.95, w: 12.1, cols: 3, rowH: 1.62, items: [
    ...
    { term: "HITL", def: "людина в ланцюзі підтверджує незворотне (Тема 8)" },
  ] });
...

{
  ...
  D.tile(s, { x: MX, y: 1.9, w: 5.85, h: 1.35, title: "Поки модель говорить",
    body: "найгірше — погана відповідь: користувач перепитає, оператор виправить", tone: "good" });
  D.tile(s, { x: 6.87, y: 1.9, w: 5.85, h: 1.35, title: "Щойно модель діє",
    body: "вона може зробити щось. І двічі. Або не те. Або те — але в невдалий момент", tone: "crit" });
  D.tile(s, { x: MX, y: 3.4, w: 5.85, h: 1.35, title: "Tool-рантайм = production surface",
    body: "схеми, таймаути, ретраї, логи і права — як у будь-якого API", tone: "acc" });
  D.tile(s, { x: 6.87, y: 3.4, w: 5.85, h: 1.35, title: "LLM = ненадійний клієнт Вашого API",
    body: "дивні аргументи, повтори, «передумав» посередині: клієнта валідують", tone: "acc" });
  D.flow(s, { x: MX, y: 4.95, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "погана відповідь → перепитати", tone: "good", sub: "слово можна перечитати" }, { label: "create_ticket ×2 → два тікети", tone: "crit", sub: "дію не завжди можна відкотити" },
    { label: "схема · таймаут · ключ · лог", tone: "acc", sub: "контур навколо дії" }] });
  D.band(s, { x: MX, y: 5.85, w: 12.1, h: 0.85, tone: "acc", label: "Навіщо цей шар",
...

{
  ...
  D.band(s, { x: MX, y: 5.35, w: 12.1, h: 1.05, tone: "good",
    text: "Модель нічого не виконала — вона запропонувала. Виконувати чи ні вирішує Ваш сервіс, і саме там живуть усі запобіжники теми." });
}

{
  ...
  D.band(s, { x: MX, y: 3.8, w: 12.1, h: 1.15, tone: "crit", label: "Жодних «спробуємо якось»",
  ...
  D.flow(s, { x: MX, y: 5.15, w: 12.1, h: 0.56, size: 10.5, items: [
    { label: "модель: lookup_order без номера", tone: "crit" }, { label: "валідація за схемою", tone: "acc" },
    { label: "відхилити + залогувати", tone: "good" }, { label: "вгадати за контекстом ✕", tone: "crit", dashed: true }] });
  D.band(s, { x: MX, y: 6.05, w: 12.1, h: 0.6, tone: "acc",
...

{
  ...
  D.band(s, { x: MX, y: 4.62, w: 12.1, h: 1.25, tone: "acc", label: "Реєстр інструментів стає зовнішнім (MCP)",
    text: "Чужі сервіси публікують інструменти за стандартним протоколом — а Ви вирішуєте, що дозволено, з якими лімітами і що потребує людини. Обв'язка теми не змінюється." });
...

{
  ...
  D.band(s, { x: MX, y: 3.8, w: 12.1, h: 1.55, tone: "crit", label: "Чесна «дірка» сьогоднішньої теми",
...

{
  ...
  D.layers(s, { x: MX, y: 1.95, w: 12.1, h: 0.78, gap: 0.14, items: [
    ...
    { label: "Що", body: "назва інструмента — поле tool у відповіді /chat; колонка в requests — Ваш перший крок аудиту", tone: "acc" },
  ...
  D.band(s, { x: MX, y: 5.55, w: 12.1, h: 0.95, tone: "warn", label: "Виняток кешу",
    text: "Відповіді з tool-викликами не кешуються: статус замовлення, закешований на годину, — впевнена брехня. Межа з Теми 5 тепер Ваша." });
}
...

{
  ...
  [["сира відповідь mock", "tool_calls: назва, finish_reason (аргументи у mock порожні)"],
  ...
  ].forEach(([t, b], i) => D.tile(s, { x: MX + (i % 2) * 4.15, y: 1.9 + Math.floor(i / 2) * 1.8, w: 3.95, h: 1.6, badge: i + 1, title: t, body: b, tone: i >= 2 ? "crit" : "good" }));
  D.chatMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.4, msgs: [
    { who: "user", text: "Де моє замовлення #10482?" }, { who: "bot", text: "Перевіряю статус… Оплачено, доставку призначено.", badge: "tool: lookup_order · read-only ✓", tone: "good" },
    { who: "user", text: "Хочу повернути гроші" }, { who: "bot", text: "Створив тікет #1042.", badge: "tool: create_ticket · без людини ✕", tone: "crit" }] });
  D.band(s, { x: MX, y: 5.55, w: 12.1, h: 1.15, tone: "acc", label: "Навіщо",
    text: "Побачити межу власними очима: читання виконується спокійно, а незворотна дія двічі створює зобов'язання — обидві дірки закриваються у Темі 8." });
}

{
  ...
  [["Прочитати tool_calls", "сира відповідь mock: структура, назва, finish_reason", false],
   ...
   ["Вирішити таймаут і ключ", "одне-два речення в описі PR; реалізація обох — опційно", false],
...

{
  ...
  D.tile(s, { x: MX, y: 1.9, w: 7.9, h: 1.15, title: "Прохання ≠ подія", body: "модель просить, сервіс вирішує і виконує — межа видима в коді", tone: "good" });
  D.tile(s, { x: MX, y: 3.2, w: 7.9, h: 1.15, title: "Читання безпечне", body: "повтор lookup_order коштує зайвий запит у базу — і все", tone: "acc" });
  D.tile(s, { x: MX, y: 4.5, w: 7.9, h: 1.15, title: "Незворотне — ні", body: "два тікети на одне питання: дірка HITL і відсутність ідемпотентності", tone: "crit" });
  D.chatMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.75, msgs: [
    { who: "user", text: "Хочу повернути гроші" }, { who: "bot", text: "Створив тікет #1042.", badge: "create_ticket → #1042", tone: "warn" },
    { who: "user", text: "Хочу повернути гроші" }, { who: "bot", text: "Створив тікет #1043.", badge: "create_ticket → #1043 · два тікети ✕", tone: "crit" }] });
  D.band(s, { x: MX, y: 5.85, w: 12.1, h: 0.8, tone: "card",
    text: "Різниця між «попросила» і «зробила» — суть цієї теми. У Темі 8 між ними стане людина." });
}

{
  ...
  D.checklist(s, { x: MX + 0.45, y: 2.3, w: 11.3, cols: 2, h: 3.20, size: 14, items: [
    ...
    "рішення про таймаут і ключ операції записане в описі PR",
...

{
  ...
  s.addText("обидві теми тижня здаються одним PR: кеш із лічильниками з Теми 5 плюс сьогоднішнє безпечне виконання інструментів.",
...
D.closingSlide({
  summary: [
    "tool_calls — прохання моделі; вирішує і виконує Ваш сервіс",
    ...
    "незворотне не виконується автономно — контракт зафіксовано, механізм — у Темі 8",
  ],
  nextTitle: "Наступний крок → Тема 7 · Reliability: fallback, деградація і circuit breaker",
