...

{
  ...
  [["Зробити збій значенням", "виклик у функції, де помилка — не виняток"],
   ...
   ["Віддати ввічливу заглушку", "замість помилки 500, з чесним 503 у лозі"],
...

{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [3.1, 4.5, 4.5], rowH: 0.56, size: 11.5,
    ...
    rows: [
      ...
      { cells: ["Коли доречний", "минущі збої: мережева помилка, 429", "провайдер недоступний або деградує"] },
...

{
  ...
  // таблиця вище: y=1.90 + 5 рядків × 0.62 = низ 5.00 — плашка мусить бути нижче
  D.band(s, { x: MX, y: 5.2, w: 12.1, h: 1.45, tone: "good", label: "Принцип",
...

{
  ...
  D.table(s, { x: MX, y: 1.9, w: 12.1, colW: [1.0, 6.3, 4.8], rowH: 0.58, size: 11.5,
    ...
    rows: [
      ...
      { cells: ["2", "відповідь іншої моделі (Тема 3)", "можливо, трохи іншу манеру"], tone: "acc" },
      { cells: ["3", "відповідь моделі без звернень до зовнішніх систем", "актуальні дані"] },
      { cells: ["4", "раніше згенеровану відповідь на схоже питання (Тема 5)", "свіжість і персоналізацію"] },
...

{
  ...
  D.band(s, { x: MX, y: 4.78, w: 12.1, h: 1.15, tone: "good",
    text: "«Скільки запитів пішло через fallback у п'ятницю?» — з цього починається постмортем." });
  D.band(s, { x: MX, y: 6.08, w: 12.1, h: 0.62, tone: "card",
    text: "На тижні 5 це стане плитками консолі — сьогодні досить, що цифри є." });
}
...
// ─── дві сучасні поправки до бюджету часу (доважок блоку 09) ───
{
  ...
  D.tile(s, { x: MX, y: 2.15, w: 5.85, h: 1.6, badge: 1, title: "Режим мислення — інший порядок часу",
    body: "таймаут під швидкий режим падає на легітимно повільній відповіді", tone: "warn" });
  D.tile(s, { x: 6.87, y: 2.15, w: 5.85, h: 1.6, badge: 2, title: "Ліміти стали багатовимірними",
    body: "не лише запити на хвилину: токени, окремі квоти на «думаючі» моделі", tone: "warn" });
  D.bars(s, { x: MX, y: 3.95, w: 12.1, labelW: 3.4, noteW: 3.8, rowH: 0.34, gap: 0.1, rows: [
    { label: "швидкий режим", value: 1, note: "таймаут розрахований під нього", tone: "good" },
    { label: "режим мислення", value: 8, note: "інший порядок часу — таймаут спрацює", tone: "crit" },
    { label: "квота на токени / хв", value: 8, note: "вичерпується раніше за квоту на запити", tone: "crit" }] });
  D.band(s, { x: MX, y: 5.3, w: 12.1, h: 0.6, tone: "crit",
    text: "429 приходить не тому, що «багато запитів», а тому, що Ви вибрали токен-квоту." });
  D.band(s, { x: MX, y: 6.02, w: 12.1, h: 0.62, tone: "good",
    text: "Бюджет часу — властивість маршруту, а не константа сервісу." });
}

// ─── РОЗДІЛЮВАЧ · практика ───
{
  D.divider({ big: "ПРАКТИКА", sub: "стенд наживо — і чесна межа того, що він доводить",
    pill: "Лабораторна: три кроки + опційний", notes: N() });
}

{
  const s = D.slide({ title: "Зараз Ви побачите — і навіщо", pill: "do", notes: N() });
  [["__fail_503 → заглушка", "ввічлива відмова замість помилки 500", "good"],
  ...
  ].forEach(([t, b, tone], i) => D.rows(s, { x: MX, y: 1.85 + i * 0.66, w: 7.9, rowH: 0.56, items: [{ n: i + 1, title: t, body: b, tone: tone === "crit" ? "crit" : tone === "warn" ? "warn" : undefined }] }));
  D.chatMock(s, { x: 8.92, y: 1.85, w: 3.8, h: 3.35, msgs: [
    { who: "user", text: "__fail_503" }, { who: "bot", text: "Вибачте, тимчасові проблеми на нашому боці. Спробуйте, будь ласка, трохи згодом.", badge: "fallback → заглушка · 503 у лозі · 12 мс", tone: "crit" },
    { who: "user", text: "Як скинути пароль?" }, { who: "bot", text: "Надішліть запит на email підтримки.", badge: "200 · чат живий ✓", tone: "good" }] });
...

{
  ...
  D.tile(s, { x: MX, y: 1.9, w: 7.9, h: 1.1, title: "Збій швидкий", body: "мілісекунди замість восьми секунд прихованих ретраїв", tone: "good" });
  D.tile(s, { x: MX, y: 3.15, w: 7.9, h: 1.1, title: "Користувач бачить ввічливість", body: "заглушка замість помилки 500 — чат живий", tone: "acc" });
  D.tile(s, { x: MX, y: 4.4, w: 7.9, h: 1.1, title: "Система бачить правду", body: "503 у лозі й лічильник переходів, а не «все добре»" });
  D.windowMock(s, { x: 8.92, y: 1.9, w: 3.8, h: 3.6, caption: "лог · requests", lines: [
    "SELECT status, latency_ms", "  FROM requests LIMIT 4;", "", { t: " 503 |   12 ms   ← __fail_503", c: "crit" }, { t: " 503 |    9 ms   ← __fail_503", c: "crit" },
    { t: " 200 | 1180 ms", c: "good" }, { t: " 200 | 1204 ms", c: "good" }, "", { t: "fallback_events: 2", c: "warn" }, { t: "500 користувачу: 0 ✓", c: "good" }] });
  D.band(s, { x: MX, y: 5.7, w: 12.1, h: 1.0, tone: "warn", label: "Межа стенда — чесна",
...
D.closingSlide({
  ...
  nextTitle: "Наступний крок → Тема 8 · Safety, guardrails і human-in-the-loop",
  nextBody: "Система переживає збій провайдера. У наступній темі — про збої іншого роду: коли шкоду завдає не інфраструктура, а зміст. Injection, витік даних і незворотні дії, які мусить підтвердити людина — та сама «дірка», яку ми лишили відкритою у Темі 6.",
  notes: N(),
});
